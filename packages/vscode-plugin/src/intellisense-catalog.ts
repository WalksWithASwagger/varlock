...
export const ROOT_DECORATORS: Array<DecoratorInfo> = [
  ...
  },
  {
    ...
    documentation: [
      ...
      'Common args: `path`, `auto` (default true; set false to generate only via `varlock codegen`), `executeWhenImported` (run even when this file is imported).',
      'TS options: `exposeEnv` (`global` | `local` | `none`) controls where the coerced `ENV` object lives;',
    ...
    insertText: '@generateTsTypes(path=${1:./env.d.ts}${2:, auto=${3|true,false|}}${4:, executeWhenImported=${5|false,true|}})',
    isFunction: true,
  },
  {
    ...
    documentation: 'Example: `# @generatePythonEnv(path=./env.py)`. Common args: `path`, `auto`, `executeWhenImported`.',
    insertText: '@generatePythonEnv(path=${1:./env.py}${2:, auto=${3|true,false|}}${4:, executeWhenImported=${5|false,true|}})',
    isFunction: true,
  },
  {
    ...
    documentation: 'Example: `# @generateRustEnv(path=./src/env.rs)`. Common args: `path`, `auto`, `executeWhenImported`.',
    insertText: '@generateRustEnv(path=${1:./src/env.rs}${2:, auto=${3|true,false|}}${4:, executeWhenImported=${5|false,true|}})',
    isFunction: true,
  },
  {
    name: 'generateGoEnv',
    ...
    documentation: 'Example: `# @generateGoEnv(path=./env/env.go)`. The package name defaults to the output directory; override with `package=`. Common args: `path`, `auto`, `executeWhenImported`.',
    insertText: '@generateGoEnv(path=${1:./env/env.go}${2:, auto=${3|true,false|}}${4:, executeWhenImported=${5|false,true|}})',
    isFunction: true,
  },
  {
    name: 'generatePhpEnv',
    ...
    documentation: 'Example: `# @generatePhpEnv(path=./Env.php)`. For Composer/PSR-4, set `namespace=` and/or `class=` (default: global `Env`). Common args: `path`, `auto`, `executeWhenImported`.',
    insertText: '@generatePhpEnv(path=${1:./Env.php}${2:, auto=${3|true,false|}}${4:, executeWhenImported=${5|false,true|}})',
    isFunction: true,
  },
  {
    name: 'generateTypes',
    ...
    documentation: 'Kept as a TypeScript-only alias. Example: `# @generateTypes(lang=ts, path=./env.d.ts)`. Also accepts common codegen args `auto` and `executeWhenImported`.',
    insertText: '@generateTypes(lang=ts, path=${1:./env.d.ts}${2:, auto=${3|true,false|}}${4:, executeWhenImported=${5|false,true|}})',
    isFunction: true,
  ...
  },
  {
    ...
    documentation: [
      'Takes a path as the first positional arg.',
      'Named args: `enabled` (conditionally import), `allowMissing` (skip if path missing),',
      '`pick` / `omit` to filter imported keys (globs supported; cannot combine pick and omit).',
    ].join(' '),
    insertText: '@import(${1:./.env.shared}${2:, allowMissing=${3|false,true|}}${4:, pick=[${5:KEY_*}]})',
    isFunction: true,
  },
  {
    name: 'plugin',
  ...
  },
  {
    name: 'cache',
    scope: 'root',
    summary: 'Sets global cache mode for cache() and plugin cache APIs.',
    documentation: 'Valid values: `auto` (default), `memory`, `disk`, or `disabled`. Can also use a resolver like `forEnv(dev, "disk")`.',
    insertText: '@cache=${1|auto,memory,disk,disabled|}',
  },
  {
    name: 'redactLogs',
    scope: 'root',
  ...
  },
  {
    ...
    scope: 'root',
    summary: 'Encrypts the injected env blob in server-side build output.',
    documentation: 'Boolean (or `forEnv(...)`). When true, encrypts with AES-256-GCM using `_VARLOCK_ENV_KEY`. Primarily for serverless deployments.',
    insertText: `@encryptInjectedEnv=${booleanChoiceSnippet()}`,
  },
  {
    name: 'disableProcessEnvInjection',
    scope: 'root',
    summary: 'Prevents injecting resolved values into process.env.',
    documentation: 'Must be a static boolean. When true, values are only available via the ENV proxy. Affects `@generateTsTypes` processEnv defaults.',
    insertText: `@disableProcessEnvInjection=${booleanChoiceSnippet()}`,
  },
  {
    name: 'setValuesBulk',
    scope: 'root',
    ...
    documentation: [
      'First arg is the data resolver (returns a string).',
      'Named args: `format` (`json` | `env`), `createMissing` (create unknown keys; usually avoid),',
      '`enabled` (conditionally run), `pick` / `omit` key filters (globs supported).',
    ].join(' '),
    insertText: '@setValuesBulk(${1:opLoadEnvironment(env-id)}${2:, format=${3|json,env|}}${4:, createMissing=${5|false,true|}}${6:, pick=[${7:API_*}]})',
    isFunction: true,
  },
  {
...
export const ITEM_DECORATORS: Array<DecoratorInfo> = [
  ...
  },
  {
    name: 'internal',
    scope: 'item',
    summary: 'Marks an item as internal to varlock only.',
    documentation: [
      'Resolved and usable by other items (e.g. secret-zero credentials), but excluded from injection,',
      'the serialized env blob, and generated types. Use `@internal=false` to opt a default-internal type back in.',
    ].join(' '),
    insertText: '@internal',
  },
  {
  ...
  },
  {
    name: 'deprecated',
    scope: 'item',
    summary: 'Marks an item as deprecated.',
    documentation: 'Bare `@deprecated` or `@deprecated=true` flags the item. Pass a string message with `@deprecated="use OTHER instead"`. Use `@deprecated=false` to opt out.',
...
export const RESOLVERS: Array<ResolverInfo> = [
  ...
  {
    name: 'ifs',
    summary: 'Returns the value for the first truthy condition (Excel-style IFS).',
    documentation: 'Arguments are `(condition, value)` pairs. An optional trailing default is allowed when the total arg count is odd. Returns undefined if nothing matches and there is no default.',
    insertText: 'ifs(\n  ${1:eq($$ENV, production)}, ${2:https://api.example.com},\n  ${3:eq($$ENV, staging)}, ${4:https://staging-api.example.com},\n  ${5:http://localhost:3000}\n)',
  },
  {
    name: 'not',
  ...
  {
    name: 'randomNum',
    summary: 'Generates a cryptographically random number.',
    documentation: 'Integer by default (inclusive bounds). With one arg: 0..max. With two: min..max. Pass `precision=N` for a float with N decimal places.',
    insertText: 'randomNum(${1:0}, ${2:100}${3:, precision=${4:2}})',
  },
  {
    name: 'randomUuid',
    summary: 'Generates a random UUID v4.',
    documentation: 'Often wrapped in `cache()` for a stable per-environment id.',
    insertText: 'randomUuid()',
  },
  {
    name: 'randomHex',
    summary: 'Generates a random hexadecimal string.',
    documentation: 'Default length is 32 characters. Pass `bytes=true` to treat the length as a byte count instead.',
    insertText: 'randomHex(${1:32}${2:, bytes=${3|false,true|}})',
  },
  {
    name: 'randomString',
    summary: 'Generates a random alphanumeric string.',
    documentation: 'Default length is 16 using A-Za-z0-9. Override the character set with `charset=...`.',
    insertText: 'randomString(${1:32}${2:, charset=${3:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"}})',
  },
  {
    name: 'cache',
    summary: 'Caches the result of another resolver.',
    documentation: 'First arg is the resolver to cache. Optional `ttl` (duration string or `forever`) and `key` (explicit cache key). Global mode is set with `@cache`.',
    insertText: 'cache(${1:randomUuid()}${2:, ttl=${3:"1h"}})',
  },
  {
...
/** Built-in `$VARLOCK_*` keys available without declaring them in the schema. */
export const BUILTIN_VAR_KEYS = [
  'VARLOCK_ENV',
  'VARLOCK_IS_CI',
  'VARLOCK_BRANCH',
  'VARLOCK_PR_NUMBER',
